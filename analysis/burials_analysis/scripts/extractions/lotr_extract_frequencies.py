...
save_excel(freq_deceased, "lotr_freq_deceased")
...
save_excel(freq_burialtype, "lotr_freq_burialtypes")

# Remove rows where object_category is "-"
df_objects = df[df["object_category"] != "-"]

# Frequency of objects (excluding "-")
freq_objects = df_objects.groupby("object_category").size().reset_index(name="absolute_frequency")
freq_objects["relative_frequency"] = freq_objects["absolute_frequency"] / len(df_objects)
save_excel(freq_objects, "lotr_freq_objects")
