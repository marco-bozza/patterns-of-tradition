...
output_table_path = "D:/Archivio/Documenti/Tesi/Data/burials_analysis/output/tables/freq_objects.xlsx"
output_graph_path = "D:/Archivio/Documenti/Tesi/Data/burials_analysis/output/graphs/freq_objects.png"

# === Load data ===
beowulf = pd.read_excel(os.path.join(input_path, "beowulf_freq_objects.xlsx"))
lotr = pd.read_excel(os.path.join(input_path, "lotr_freq_objects.xlsx"))
...
df_order = pd.DataFrame({"object_category": ordered_categories})

# === Merge and reorder data ===
merged = df_order.merge(
    beowulf[["object_category", "absolute_frequency", "rel_freq"]],
    on="object_category", how="left"
).merge(
    lotr[["object_category", "absolute_frequency", "rel_freq"]],
    on="object_category", how="left",
...
labels = merged_plot["object_category"]
...
plt.title("Relative Frequency of Object Categories in Burial Events", fontsize=16)
