...
output_graph_path = "D:/Archivio/Documenti/Tesi/Data/singing_analysis/output/graphs/freq_singers.png"
output_table_path = "D:/Archivio/Documenti/Tesi/Data/singing_analysis/output/tables/freq_singers.xlsx"

# === Load data ===
beowulf = pd.read_excel(f"{input_path}/beowulf_freq_singers.xlsx")
lotr = pd.read_excel(f"{input_path}/lotr_freq_singers.xlsx")
...
beowulf["singer_category"] = beowulf["singer_category"].replace(rename_map)
lotr["singer_category"] = lotr["singer_category"].replace(rename_map)
...
categories_order = ["Poets", "Heroes", "Rulers", "Commoners", "G. supernat.", "E. supernat."]
all_categories = pd.DataFrame({"singer_category": categories_order})

merged = all_categories.merge(
    beowulf[["singer_category", "absolute_frequency", "relative_frequency"]],
    on="singer_category", how="left"
).merge(
    lotr[["singer_category", "absolute_frequency", "relative_frequency"]],
    on="singer_category", how="left",
...
plt.title("Relative Frequency of Singer Categories in Singing Events", fontsize=16)
